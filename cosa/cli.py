...
def show(array:Iterable[Any], indentation:int=0, enum:bool=False, start:int=1, indentor:str='\t', tail=True, head=False, file=sys.stdout) -> None: 
    """
    Print each element of an array with the desired formatting to the desired output. This will consume a generator.
    """
...
def main():
    ...
    parser.add_argument('-i', '--icon', default=None, type=str, help='Path to an image file you would like to use as the binary\'s logo (python only)')
